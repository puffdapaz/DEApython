...
def convert_keys(obj):
    ...
    if isinstance(obj,
                  dict):
        return {str(k): convert_keys(v) for k,
                        v in obj.items()}
    elif isinstance(obj,
                    list):
...
def save_to_file(obj,
                 filepath: str,
                 file_format: str):
    """Function to save DataFrame or dict to a file."""
    if isinstance(obj,
                  pd.DataFrame):
        if file_format == "csv":
            obj.to_csv(filepath,
...
def save_data(obj: Union[pd.DataFrame,
                         Dict[Any, Any]],
              filename: str,
